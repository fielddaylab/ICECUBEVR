...
public class EventPlayer : MonoBehaviour
{
        ...
        if(file.EndsWith(".txt"))
        {
          ...
          while(s != null)
          {
            if(lineCount < 2)
            {
              ...
              if(lineCount < 1) e.startPos = new Vector3(x*0.3048f,y*0.3048f,z*0.3048f);
              else              e.endPos   = new Vector3(x*0.3048f,y*0.3048f,z*0.3048f);
            }
            else
            {
              string[] data = s.Split('\t');
              if(data.Length != 7) data = s.Split(' ');
              EventData d;
  ...
  public bool IsEventPlaying()
  {
    ...
    {
      if(eventsPlaying[i].isPlaying) return true;
    }
  ...
  void StopPlaying(int e)
  {
    if(e != -1)
    {
      ...
      for(int i = 0; i < events[e].eventData.Count; ++i)
      {
        ...
        if(d) d.GetComponent<DOMController>().TurnOff();
      }
    }
  }
      ...
      if(eventsPlaying[currEventNumber].eventIndex < events[currEventNumber].eventData.Count && eventsPlaying[currEventNumber].advancedIndex)
      {
        ...
        if(d)
        {
          ...
          if(dc)
          {
            ...
            AudioSource asource = dc.GetComponent<AudioSource>();
            if(asource && asource.isActiveAndEnabled) asource.Play();
          }
        }
      }
