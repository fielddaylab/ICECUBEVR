...
{	
	...
	float gaze_t_max;
	float gaze_t_since;
	//if positive, time since entered. if negative, time since exited.
	float gaze_t_in;
	//grows to max when in, shrinks to 0 when out
	float gaze_t_run;
	//grows while not fully out. 0 when fully out
	float gaze_t_numb;
	//countdown when distance should be ignored

	float spec_t_max;
	//The time required to select a button
	float spec_t_since;
	//if positive, time since entered. if negative, time since exited.
	float spec_t_in;
	//grows to max when in, shrinks to 0 when out
	...
	void Update ()
	{
		...
		if (in_portal_motion > 0)
			in_portal_motion += Time.deltaTime * 0.8f;
		...
		if (cur_scene_i == (int)SCENE.EXTREME)
			camera_house.transform.rotation = Quaternion.Euler ((mouse_y - Screen.height / 2) * -2 + Random.Range (-1, 1), (mouse_x - Screen.width / 2) * 2 + Random.Range (-1, 1), 0 + Random.Range (-1, 1));
		else
			camera_house.transform.rotation = Quaternion.Euler ((mouse_y - Screen.height / 2) * -2, (mouse_x - Screen.width / 2) * 2, 0);
